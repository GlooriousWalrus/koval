...
pub fn ExamplePage() -> impl IntoView {
    view! {
        ...
        <Main
            id="main"
            class="max-w-[85rem] h-screen px-4 py-4 sm:px-6 lg:px-8 mx-auto text-gray-900 dark:text-gray-400"
        >
            <div>
                // <!-- Announcement Banner -->
    ...
    view! {
        <section class="max-w-[70rem] px-4 py-4 sm:px-6 lg:px-8 mx-auto">
            ...
            <div class="border rounded-xl shadow-sm  border-blue-600 dark:bg-slate-800 dark:border-slate-600 p-5 mt-3">
                <Typography variant=TypographyVariant::H3 class=TypographyClass::H3.get()>
                    Inline Block
                </Typography>
                <div class="p-4 inline-block">
                    <Button
                        class="ml-0 mx-1 bg-blue-400"
                        on_click=|e| println!("{}", e.to_string())
                    >
                        {"Default (Unstyled)"}
                    </Button>
                    <Button
                        class="ml-0 mx-1"
                        on_click=|e| web_sys::console::log_1(&e.target().unwrap())
                        variant=ButtonVariant::Solid.get()
                    >
                        {"Solid"}
                    </Button>
                    <Button
                        class="ml-0 mx-1"
                        on_click=|e| println!("{}", e.to_string())
                        variant=ButtonVariant::Outline.get()
                    >
                        {"Outline"}
                    </Button>
                    <Button
                        class="ml-0 mx-1"
                        on_click=|e| println!("{}", e.to_string())
                        variant=ButtonVariant::Ghost.get()
                    >
                        {"Ghost"}
                    </Button>
                    <Button
                        class="ml-0 mx-1"
                        on_click=|e| println!("{}", e.to_string())
                        variant=ButtonVariant::Soft.get()
                    >
                        {"Soft"}
                    </Button>
                    <Button
                        class="ml-0 mx-1"
                        on_click=|e| println!("{}", e.to_string())
                        variant=ButtonVariant::White.get()
                    >
                        {"White"}
                    </Button>
                    <Button
                        class="ml-0 mx-1"
                        on_click=|e| println!("{}", e.to_string())
                        variant=ButtonVariant::Link.get()
                    >
                        {"Link"}
                    </Button>
                </div>
                ...
                <div class="p-4 flex flex-wrap gap-2">
                    ...
                    <Button
                        class="ml-0 mx-1"
                        on_click=move |_| set_disabled(!disabled.get_untracked())
                    ...
                    <Button
                        class="ml-0 mx-1"
                        on_click=move |_| { set_disabled.update(|value| *value = !(*value)) }
                    ...
                    <Button
                        class="ml-0 mx-1"
                        on_click=move |_| set_disabled.update(|value| *value = value.not())
                        disabled=disabled
                        variant=ButtonVariant::Solid.get()
                    >
                        {move || if disabled.get() { "Disabled" } else { "Enabled" }}
        ...
        <section class="max-w-[70rem] px-4 py-4 sm:px-6 lg:px-8 mx-auto">
            ...
            <div class="border rounded-xl shadow-sm border-blue-600 dark:bg-slate-800 dark:border-slate-600 p-5 mt-3">
                <Container class="py-2">
                    <ThemeToggleButton
                        mode_fn=theme_mode
                        class="text-yellow-700 dark:text-primary-400 focus:outline-none text-sm p-1"
                ...
                <Container class="py-2">
                    <ThemeToggleSwitch
                        mode_fn=theme_mode
                        class=ToggleSwitchClassVariant::Knob.get()
                    />
                </Container>
                <Container class="py-2">
                    <ThemeToggleSwitch
